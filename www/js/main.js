...
class GameApp {
    constructor() {
        ...
        this._loadingTips = [
            ...
            'Collect all 3 Golden Cups per hole for +30% damage \u0026 +25% speed!',
            'Double-jump over a Jumper rival\u2019s arc.',
            'Watch a course champion\u2019s wind-up before committing to attacks.',
            'Shield rivals drop their guard every 2.2s \u2014 time your strikes!',
            'Long-range rivals retreat when you close in \u2014 corner them for 250 pts!',
            'Speed Boost lasts 8 seconds \u2014 use it to dodge Kamikaze rushes!',
            'Power Drive stacks with a Golden Cup set bonus for massive hits!',
        ];
    ...
    async init() {
        try {
            ...
            try { if (window.SkinManager && typeof SkinManager.initialize === 'function') SkinManager.initialize(); } catch (e) { __err('main', e); }
